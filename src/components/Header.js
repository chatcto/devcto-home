...
import { nvaData } from '@/consts/nav'
import clsx from 'clsx'
// import { ThemeSelect, ThemeToggle } from './ThemeToggle'

function Featured() {
  return (
    <Link href="/blog/61dbd68c20720">
      <a className="ml-3 text-xs leading-5 font-medium text-sky-600 dark:text-sky-400 bg-sky-400/10 rounded-full py-1 px-3 hidden xl:flex items-center hover:bg-sky-400/20">
        <strong className="font-semibold">最新博客</strong>
        <svg
          width="2"
          height="2"
          fill="currentColor"
      ...
      >
        ...
        <div className="fixed top-4 right-4 w-full max-w-xs bg-white rounded-lg shadow-lg p-6 text-base font-semibold text-gray-900 dark:bg-gray-800 dark:text-gray-400 dark:highlight-white/5">
          ...
          <ul className="space-y-6">
            ...
            <li>
              <a
                href="https://github.com/wsdo/wsdo"
...
const nvaItemData = [
  { name: '博客', url: '/blog' },
  { name: '开发环境', url: '/devenv/mac' },
  { name: '常用代码', url: '/frequently/introduce' },
  { name: '快捷键', url: '/shortcuts/introduce' },
  { name: '关于我', url: '/about/introduce' }
]
export function NavItems() {
  return (
    <>
      {nvaData.map((item, index) => {
        return <li key={index}>
          <Link href={item.url}>
            <a className="hover:text-sky-500 dark:hover:text-sky-400">{item.name}</a>
          </Link>
        </li>
      })}
    </>
          ...
          >
            <div className="relative flex items-center">
              <Link href="/">
                ...
                >
                  <span className="sr-only">王树东 home page</span>
